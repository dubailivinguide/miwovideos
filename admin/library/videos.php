...
class MiwovideosVideos {
	...
	public function processMp4($process, $fileType, $size) {
		...
		if (!file_exists($new_location)) {
			try {
				if (substr(PHP_OS, 0, 3) == "WIN") {
					$input = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -y -i $location -strict experimental -acodec aac -ac 2 -ab 160k -s $wxh $pad -vcodec libx264 -vpre ipod640 -b:v " . $bitrate . "k -f mp4 -threads 0 $new_location 2>&1";
					exec($input, $output);
				}
				else {
					$input = $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . " -y -i $location -strict experimental -acodec aac -ac 2 -ab 160k -s $wxh $pad -vcodec libx264 -vpre ipod640 -b:v " . $bitrate . "k -f mp4 -threads 0 $new_location 2>&1";
		...
		if (!file_exists($new_location)) {
			try {
				...
				if (substr(PHP_OS, 0, 3) == "WIN") {
					$input = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -y -i $location -strict experimental -acodec aac -ac 2 -ab 160k -s $wxh $pad -vcodec libx264 $ffpreset_libx264_slow $ffpreset_libx264_ipod640 -b:v " . $bitrate . "k -f mp4 -threads 0 $new_location 2>&1";
					exec($input, $output);
				}
				else {
					$input = $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . " -y -i $location -strict experimental -acodec aac -ac 2 -ab 160k -s $wxh $pad -vcodec libx264 $ffpreset_libx264_slow $ffpreset_libx264_ipod640 -b:v " . $bitrate . "k -f mp4 -threads 0 $new_location 2>&1";
	...
	public function processWebm($process, $fileType, $size) {
		...
		try {
			...
			if (substr(PHP_OS, 0, 3) == "WIN") {
				$command1 = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -y -i $location -s $wxh $pad $ffpreset_libvpx_720p_pass1 -b:v " . $bitrate . "k -pass 1 -an -f webm $new_location 2>&1";
				exec($command1, $output1);
				$command2 = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -y -i $location -s $wxh $pad $ffpreset_libvpx_720p_pass2 -b:v " . $bitrate . "k -pass 2 -acodec libvorbis -ab 90k -f webm $new_location 2>&1";
	...
	public function processOgg($process, $fileType, $size) {
		...
		try {
			...
			$ffpreset_libx264_ipod640 = " -coder 0 -bf 0 -refs 1 -flags2 -wpred-dct8x8 -level 30 -maxrate 10000000 -bufsize 10000000 -wpredp 0 ";
			if (substr(PHP_OS, 0, 3) == "WIN") {
				$command = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -y -i $location -s $wxh $pad -vcodec libtheora -b:v " . $bitrate . "k -acodec libvorbis $new_location 2>&1";
				exec($command, $output);
			}
			else {
				$command = $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . " -y -i $location -s $wxh $pad -vcodec libtheora -b:v " . $bitrate . "k -acodec libvorbis $new_location 2>&1";
	...
	public function getTitle($process) {
		...
		// Get information on original
		if (substr(PHP_OS, 0, 3) == "WIN") {
			$command = "\"" . $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . "\" -i $location -f ffmetadata " . MPATH_CACHE . "/metadata" . $row->id . ".ini 2>&1";
			exec($command, $output);
		}
		else {
			$command = $config->get('ffmpeg_path', '/usr/bin/ffmpeg') . " -i $location -f ffmetadata " . MPATH_CACHE . "/metadata" . $row->id . ".ini 2>&1";
		...
		if (file_exists($location)) {
			...
			try {
				if (substr(PHP_OS, 0, 3) == "WIN") {
					...
					//$log->input = "\"".$config->get('ffmpeg_path', '/usr/bin/ffmpeg')."\" -y -b:v 700k -qscale 0 -b:v 1500k -i $location -vf \"movie=".$logo." [logo];[in][logo] overlay=".$overlay." [out]\" -vcodec $vcodec -acodec copy -f $format $new_location 2>&1";
	...
	public function convertToHtml5($video_id = null, $filename = null) {
		...
		if (!$this->processMp4($process, 'mp4', $size, 1)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_MP4_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_MP4_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->processWebm($process, 'webm', $size, 1)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_WEBM_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_WEBM_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->processOgg($process, 'ogg', $size, 1)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_OGG_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_OGG_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->processThumb($process, 'jpg', $size, 1)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_THUMB_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_THUMB_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->getDuration($process)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_GET_DURATION_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_GET_DURATION_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->getTitle($process)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_GET_TITLE_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_GET_TITLE_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if (!$this->checkMoovAtoms($process)) {
			$json['error'] = MText::_('COM_MIWOVIDEOS_ERROR_CHECK_MOOV_ATOMS_PROCESSING');
			MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_CHECK_MOOV_ATOMS_PROCESSING'));
			echo json_encode($json);
			return false;
		}
		if ($this->config->get('frames')) {
			if (!$this->processFrames($process, MIWOVIDEOS_UPLOAD_DIR."/videos/".$video_id."/orig/".$filename)) {
				...
				MiwoVideos::log(MText::_('COM_MIWOVIDEOS_ERROR_FRAMES_PROCESSING'));
				echo json_encode($json);
				return false;
			}
		}

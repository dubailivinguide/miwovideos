	    ...
	    if ($this->config->get('upload_script') == 'fancy' and !empty($filedata)) {
		    if (!$this->config->get('auto_process_videos')) {
			    ...
			    if (substr(PHP_OS, 0, 3) != "WIN") {
				    ...
				    @exec("env -i ".$this->config->get('php_path', '/usr/bin/php')." $cli convertToHtml5 ".$row->id." ".$filename." > /dev/null 2>&1  &", $output, $error);
			    }
			    else {
				    @exec('where php.exe', $php_path);
				    // @TODO Log if throw an error
				    @exec($this->config->get('php_path', $php_path)." $cli convertToHtml5 ".$row->id." ".$filename." NUL", $output, $error);
			    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ($_FILE_DATA as $arrayKey => $slot) {
	        ...
	        if (!$config->get('auto_process_videos')) {
		        ...
		        if (substr(PHP_OS, 0, 3) != "WIN") {
			        ...
			        @exec('where php.exe', $php_path);
			        // @TODO Log if throw an error
			        @exec($config->get('php_path', $php_path)." $cli convertToHtml5 ".$row->id." ".$filename." NUL", $output, $error);
		        }
    ...
    public function addProcesses($item) {
        ...
        if ($this->config->get('process_videos') == 0) return true;
        ...
        //$processes[] = $process_lib->add($item, '22', 1); // Inject Metadata
        ...
        if (substr(PHP_OS, 0, 3) != "WIN") {
            // @TODO Log if throw an error
            @exec("env -i " . $this->config->get('php_path', '/usr/bin/php') . " $cli process " . implode(" ", $processes) . " > /dev/null 2>&1  &", $output, $error);
        }
        else {
            @exec('where php.exe', $php_path);
            // @TODO Log if throw an error
            @exec($config->get('php_path', $php_path)." $cli process ".implode(" ", $processes)." NUL", $output , $error);
        }

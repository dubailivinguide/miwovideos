...
if($MULTI_CONFIGS_ENABLED){
	...
	else{ showAlertMessage("ERROR: Failed to find config_file parameter", 1); }
}
...
// Create directories
if(!create_dir($TEMP_DIR)){ showAlertMessage("ERROR: Failed to create temp_dir: $TEMP_DIR", 1); }
if(!create_dir($_CONFIG['upload_dir'])){ showAlertMessage("ERROR: Failed to create upload_dir: " . $_CONFIG['upload_dir'], 1); }
if($_CONFIG['log_uploads']){
	if(!create_dir($_CONFIG['log_dir'])){ showAlertMessage("ERROR: Failed to create log_dir: " . $_CONFIG['log_dir'], 1); }
}
...
else{ showAlertMessage("ERROR: Failed to open link file: $UPLOAD_ID.link", 1); }
...
function purge_ubr_files($temp_dir, $purge_time_limit, $file_type){
	$now_time = time(); //mktime() gives an error
	...
	if(is_dir($temp_dir)){
		...
		else{ showAlertMessage("ERROR: Failed to open temp_dir: $temp_dir", 1); }
	}
	else{ showAlertMessage("ERROR: Failed to find temp_dir: $temp_dir", 1); }
}

...
class MiwovideosModelChannels extends MiwovideosModel {
    ...
    public function getTotal() {
        if (empty($this->_total)) {
            $this->_total = MiwoDB::loadRow("SELECT COUNT(DISTINCT c.id) FROM #__miwovideos_{$this->_table} AS c RIGHT JOIN #__miwovideos_videos AS v ON (v.channel_id = c.id)".$this->_buildViewWhere());
        }

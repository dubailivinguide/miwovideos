...
//***************************************************************************************************************
// * MiwoVideos MODIFICATION *
// 1) All instances of "form_upload" replaced with "ubr_upload" (this gives Joomla compatibility)
...
function checkFileNameFormat(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.ubr_upload.elements['upfile_' + i].value != ""){
			var string = document.ubr_upload.elements['upfile_' + i].value;
...
function checkAllowFileExtensions(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.ubr_upload.elements['upfile_' + i].value != ""){
			if(!document.ubr_upload.elements['upfile_' + i].value.match(allow_extensions)){
				var string = document.ubr_upload.elements['upfile_' + i].value;
				var num_of_last_slash = string.lastIndexOf("\\");
...
function checkDisallowFileExtensions(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.ubr_upload.elements['upfile_' + i].value != ""){
			if(document.ubr_upload.elements['upfile_' + i].value.match(disallow_extensions)){
				var string = document.ubr_upload.elements['upfile_' + i].value;
				var num_of_last_slash = string.lastIndexOf("\\");
...
function checkNullFileCount(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.ubr_upload.elements['upfile_' + i].value == ""){ null_file_count++; }
	}
...
function iniFilePage(){
	...
	for(var i = 0; i < upload_range; i++){
		document.ubr_upload.elements['upfile_' + i].disabled = false;
		document.ubr_upload.elements['upfile_' + i].value = "";
	}

	document.getElementById('upload_button').disabled = false;
	document.getElementById('progress_bar').style.display = "none";
	document.ubr_upload.reset();
}
...
function resetProgressBar(){
	...
	if(show_current_position){ document.getElementById('currentupld').innerHTML = 0; }
...
function linkUpload(){
	...
	{
		document.getElementById("ubr_upload").target = "upload_iframe";
	}
	...
	if(show_files_uploaded){
		...
		for(var i = 0; i < upload_range; i++){
			if(document.ubr_upload.elements['upfile_' + i].value != ""){ total_uploads++; }
		}
	...
	else{ jsel.src = path_to_link_script + '&item_id='+ item_id +'&rnd_id=' + day.getTime(); }
...
function startUpload(upload_id, debug_upload){
	...
	document.ubr_upload.action = path_to_upload_script + '?upload_id=' +  upload_id;
	document.ubr_upload.submit();

	for(var i = 0; i < upload_range; i++){ document.ubr_upload.elements['upfile_' + i].disabled = true; }
...
function setProgressStatus(bytes_read, lapsed_time, uploaded_files){
	...
	if(cedric_progress_bar == 1){
		...
		if(show_current_position){
			dom = document.getElementById('currentupld');
...
function smoothCedricBytes(){
	if(info_bytes < total_Kbytes && !cedric_hold){
		info_bytes++;
		var dom = document.getElementById('currentupld');

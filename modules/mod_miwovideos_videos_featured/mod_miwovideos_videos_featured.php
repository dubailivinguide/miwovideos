...
$sql = 'SELECT * FROM #__miwovideos_videos WHERE published = 1 AND featured = 1'
    .' AND access IN ('.implode(',', $user->getAuthorisedViewLevels()).')'.$extraWhere.' ORDER BY created DESC '.($numberVideos ? ' LIMIT '.$numberVideos : '');

$db->setQuery($sql) ;
$rows = $db->loadObjectList() ;

require(MModuleHelper::getLayoutPath('mod_miwovideos_videos_featured', 'default'));